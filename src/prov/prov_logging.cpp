...
void PROV::routeLogByRef(LOG_TYPE _type, std::string *_msg)
{
    if (xSemaphoreTake(semProvRouteLock, portMAX_DELAY)) // We use this lock to prevent sys_evt and wifi_run tasks from having conflicts
    {
...
void PROV::routeLogByValue(LOG_TYPE _type, std::string _msg)
{
    if (xSemaphoreTake(semProvRouteLock, portMAX_DELAY)) // We use this lock to prevent sys_evt and wifi_run tasks from having conflicts
    {
        LOG_TYPE type = _type; // Copy our parameters upon entry before they are over-written by another calling task.
