...
void SNTP::runEvents()
{
    ...
    if (lockGetUint8(&sntpEvents) & _sntpTimeUpdated) // We just received a time sync notification.  Mark the time valid and print out the time.
    {
        ...
        // The runEvents function lags the real event by about 1/4 second.  If you watch the SNTP update the time every hour, you'll
        // notice that this reading lags 1 second about every 4 hours.
        // if (show & _showEvents)
